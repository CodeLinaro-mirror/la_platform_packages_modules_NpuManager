 ...
 */
typedef void (*ANpuManager_AllocCallback)(void* _Nullable borrowedCookie, int errorNum,
 ...
 *
 * \return A new allocation request, or NULL on error.
 */
 ...
 *
 * \param request The allocation request to destroy.
 */
void ANpuManager_AllocRequest_free(ANpuManager_AllocRequest* _Nonnull request) __INTRODUCED_IN(37);
 ...
 *
 * To clear the cookie, set both cookieToOwn and cookieDeleter to NULL.
 *
 * If never called, the default cookieToOwn and cookieDeleter are NULL.
 *
 * The deleter may be called from any thread. The user is responsible for
 * deleting the cookie or decrementing the associated reference counts in a thread-safe
 * manner.
 ...
 * \param cookieToOwn The user cookie to be owned by ANpuManager.
 ...
 *
 * Caller must not use pointers in \a buffers after this call to prevent
 * use-after-free bugs.
 *
 ...
 *         set. If the buffer was preempted, errno is set to ENOMEM.
 *
 ...
 *         errno set to ENOMEM, even though when ANpuManager_OnPreempt has not
 *         been called yet.
 */
